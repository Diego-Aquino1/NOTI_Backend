from shared.models.inc_incidents import IncIncident, IncIncidentAddress, GeoLocation
...
def obtener_detalles(url_completa, titulo):
    try:
        response_detalle = requests.get(url_completa, timeout=10, verify=False)
...
async def obtener_cortes(session: AsyncSession):
    response = requests.get(url_base, timeout=10, verify=False)
    resultado = []
